...
class GenericSwitchConfigurationHandler:
    ...
    MQTT_TOPIC_SET_SWITCH_STATE = None  # type: str
    MQTT_TOPIC_REPORT_SWITCH_STATE = None  # type: str
    SWITCH_CONTROL_OUTPUT_PIN = None  # type: int
    SWITCH_CONTROL_OUTPUT_ACTIVE = None  # type: bool

    _hexdigits = '0123456789abcdef'

    def generate_client_id(self):
        self.MQTT_CLIENT_ID = 'generic_switch_' + ''.join(choice(self._hexdigits) for _ in range(32))
    ...
    def __init__(self, config_file):
        ...
        self.MQTT_TOPIC_SET_SWITCH_STATE = self.\
            str_parse(config['MQTTTopicConfig']['mqtt_topic_set_switch_state'],
                      'mqtt_topic_set_switch_state',
                      False)
        self.MQTT_TOPIC_REPORT_SWITCH_STATE = self.\
            str_parse(config['MQTTTopicConfig']['mqtt_topic_report_switch_state'],
                      'mqtt_topic_report_switch_state',
                      False)
        self.SWITCH_CONTROL_OUTPUT_PIN = self.\
            int_parse(config['GPIOConfig']['switch_control_output_pin'],
                      'switch_control_output_pin',
                      False)
        self.SWITCH_CONTROL_OUTPUT_ACTIVE = self.\
            bool_parse(config['GPIOConfig']['switch_control_output_active'],
                       'switch_control_output_active',
                       False)
